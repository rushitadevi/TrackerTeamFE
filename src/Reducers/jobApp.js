          ...
          case "ADD_JOB_APP":
            ...
            case "GET_JOB_APPS":
              return {
                ...
                closed:{ items: action.payload.filter(x => x.status === "applicationWithdrawn" || x.status === "rejected"), count: action.payload.filter(x => x.status === "application withdrawn" || x.status === "rejected").length},
