...
const mapDispatchToProps = dispatch => ({
  ...
  getEntryLevelThunk: () => dispatch(getEntryLevel()),
...
class StudentDashboard extends React.Component {
  ...
  searchInput = async e => {
    ...
    if (e.currentTarget.value.length >= 4) {
      await this.props.getEntryLevelThunk(e.currentTarget.value);
    } 
    // else {
    //   await this.props.getEntryLevelThunk("");
    // }
    ...
    return (
      <Container fluid className="studentHomepage">
        ...
        <Container className="filterBar">
          ...
          <Input className="searchCompany" id="search" placeholder="Company" />

          <i className="material-icons" id="roleIcon">
            work_outline
          </i>
          <Input id="jobRole" placeholder="Role" />
          ...
          <select className="form-control" id="entryLevel" name="role"  value={this.state.role} onChange={(val) => this.setState({ role: val.currentTarget.value })}>
         }
